...
class Encoder(nn.Module):
    """
    First convolutional part of the Orientation‑boosted Voxel Net (ORION) for 3D object recognition.
...
class HeadContrastive(nn.Module):
    """
    1 hidden layer MLP
    returns a 128 dimensional representetion vector
    """
...
class HeadClassification(nn.Module):
    """
    1 hidden layer MLP
    returns a 10 dimensional vector unnormalized for the cross entropy loss
    """
...
class NTXentLoss():
    ...
    def __call__(self, z_q, z_plus):
        ...
        # 4) Mask out self‑similarities, put to -inf so that log(zi*zi)=0
